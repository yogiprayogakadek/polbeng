...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title fw-semibold mb-4">Departments List</h5>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button"
                                        ...
                                        data-id="{{ $data->id }}" data-name="{{ $data->department_name }}"
                                        data-status="{{ $data->is_active ? 'disable' : 'activate' }}"
                                        data-url="{{ route('department.toggleStatus', $data->id) }}"
                                        data-bs-toggle="tooltip" data-bs-custom-class="custom-tooltip"
                                        data-bs-placement="top"
                                        data-bs-title="{{ $data->is_active ? 'Disable Department' : 'Activate Department' }}">
                                    ...
                                    <button type="button" class="btn bg-danger-subtle text-danger btn-delete"
                                        data-id="{{ $data->id }}" data-name="{{ $data->department_name }}"
                                        data-url="{{ route('department.destroy', $data->id) }}" data-bs-toggle="tooltip"
                                        data-bs-placement="top" data-bs-title="Delete Department">
...
@push('script')
    ...
    <script>
        ...
        $(document).on('click', '.btn-toggle-status', function(e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                html: `Want to <strong>${text} ${name}</strong> department?`,
                icon: icon,
        ...
        $(document).on('click', '.btn-delete', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                html: `The <strong>${name}</strong> Department will deleted.`,
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function(err) {
                            Swal.fire('Gagal!', 'Something when wrong.',
                                'error');
                        }
                    });
                }
