...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title fw-semibold mb-4">Project Categories List</h5>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button"
                                        ...
                                        data-url="{{ route('projectCategory.toggleStatus', $data->id) }}"
                                        data-bs-toggle="tooltip" data-bs-custom-class="custom-tooltip"
                                        data-bs-placement="top"
                                        data-bs-title="{{ $data->is_active ? 'Disable Project Category' : 'Activate Project Category' }}">
                                    ...
                                    <a href="{{ route('projectCategory.edit', $data->id) }}">
                                    ...
                                    <button type="button" class="btn bg-danger-subtle text-danger btn-delete"
                                        data-id="{{ $data->id }}" data-name="{{ $data->project_category_name }}"
                                        data-url="{{ route('projectCategory.destroy', $data->id) }}"
                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                        data-bs-title="Delete Study Program">
...
@push('script')
    ...
    <script>
        ...
        $(document).on('click', '.btn-toggle-status', function(e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                html: `Want to <strong>${text} ${name}</strong> Project category?`,
                icon: icon,
        ...
        $(document).on('click', '.btn-delete', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                html: `The <strong>${name}</strong> Project category will deleted.`,
                icon: 'warning',
                showCancelButton: true,
